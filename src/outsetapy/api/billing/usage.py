...
class Usage:
  ...
  async def get_all(self, options: dict = {}) -> List[UsageItem]:
    ...
    while has_more:
      ...
      if 'Account' in options:
        request.with_params({'SubscriptionAddOn.Subscription.Account.Uid': options['Account'].Uid})
  ...
  async def add(self, usage: UsageAdd, fields: Optional[str] = None) -> Union[UsageItem, ValidationError[UsageItem]]:
    request = Request(self.store, 'billing/usage') \
      .authenticate_as_server() \
      .with_params({'fields': fields if fields else ','.join(Usage.DEFAULT_FIELDS)}) \
      .with_body(usage)

...
class Cases:
  ...
  async def get_all(self, options: dict = {}) -> List[Case]:
    ...
    results = []
    while has_more:
      request = Request(self.store, 'support/cases') \
        .with_params({'fields': options.get('fields', ','.join(Cases.DEFAULT_FIELDS))}) \
  ...
  async def add(self, new_case: dict, options: dict = {}) -> Union[Case, ValidationError[Case]]:
    request = Request(self.store, 'support/cases') \
      .with_params({'fields': options.get('fields', ','.join(Cases.DEFAULT_FIELDS))}) \
      .authenticate_as_server() \
      .with_body(new_case)
    ...
    if 'sendAutoResponder' in options:
      request.with_params({'sendAutoResponder': str(options['sendAutoResponder'])})
  ...
  async def add_reply_from_agent(self, reply: dict) -> None:
    request = Request(self.store, f"support/cases/{reply['Case']['Uid']}/replies") \
      .authenticate_as_server() \
      .with_body(reply)
  ...
  async def add_reply_from_client(self, reply: dict) -> CaseHistory:
    request = Request(self.store, f"support/cases/{reply['Case']['Uid']}/clientresponse/{reply['Comment']}") \
      .authenticate_as_server()

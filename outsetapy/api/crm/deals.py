...
class Deals:
  ...
  async def get_all(self, options: dict = {}) -> List[Deal]:
    ...
    results = []
    while has_more:
      request = Request(self.store, 'crm/deals') \
        .authenticate_as_server() \
        .with_params({
          'fields': options.get('fields', Deals.DEFAULT_FIELDS)
        })
      if 'limit' in options:
  ...
  async def get(self, uid: str, fields=None) -> Deal:
    request = Request(self.store, f'crm/deals/{uid}') \
  ...
  async def update(self, deal: DealUpdate, fields=None) -> Union[Deal, ValidationError[Deal]]:
    request = Request(self.store, f'crm/deals/{deal.Uid}') \
      .with_params({'fields': fields or Deals.DEFAULT_FIELDS}) \
      .authenticate_as_server() \
      .with_body(deal)
